...
#Mike's import code:
# Import dataset
rides <- read.csv("Nice_ride_trip_history_2016_season.csv")
...
stationDemandHours <- stationDemandHours %>%
  left_join(locations, by=c(Stationnumber = "Terminal"))
...
problemStationsdaily <- stationDemandDays %>%
  filter(Station != "Downtown Library") %>%
  filter(Rideoutminusin > 0.5*Nb.Docks | -1*Rideoutminusin > 0.5*Nb.Docks) 

problemStationsdailycounts <- problemStations %>%
  group_by(Station) %>%
  summarise(count = n())
...
stationsHighDemanddaily <- stationDemandDays%>%
  filter(Station != "Downtown Library") %>%
  filter(RidesOut > 0.25*Nb.Docks | RidesIn > 0.25*Nb.Docks) 

stationHighDemanddailycounts <- stationsHighDemand %>%
  group_by(Station) %>%
  summarise(count = n())
